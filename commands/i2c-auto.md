...
---
Meta-команда для автономного цикла разработки: создаёт все planned RFC и реализует их через code-rfc, секвенциально, в топологическом порядке зависимостей, без ручных подтверждений.
...
**Неявно включает `--auto` семантику** для всех внутренних вызовов code-rfc — отдельно передавать флаг не нужно.

**Аргументы (все опциональные):**
- `--from=rfc` (default) — создаёт все planned RFC, затем реализует каждый.
- `--from=code` — только реализация: все уже accepted RFC без IMPL прогоняются через code-rfc.
