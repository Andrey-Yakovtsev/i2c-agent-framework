...
Не требует отдельной команды. При создании нового ADR через `/i2c-create-adr` Architect может заметить новые необходимые решения — в таком случае он добавляет их в GOALS.md через Post-review подсказки Supervisor. Автоматический рост через planned-nodes в графе зависимостей — out of scope (ADR — обсуждение, не компонент).
