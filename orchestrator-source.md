...
Если `.i2c/` не существует — скажи пользователю: "Проект не инициализирован. Запусти `install.sh` из папки фреймворка: `<путь-к-фреймворку>/install.sh <путь-к-проекту>`."
...
Интерактивная конфигурация проекта после установки через `install.sh`.

> **Структура `.i2c/` уже создана `install.sh`.** Эта команда только заполняет config.md и MEMORY.md.
...
### Шаг 1 — Заполни config.md

Задай вопросы и заполни `.i2c/config.md`:
- Название проекта
- Домен
- Цель
...
Сообщи пользователю: конфигурация завершена. Следующий шаг: `/i2c-create-prd`.
...
**Шаг 2 — Discovery:** запусти Researcher (`agents/researcher.md`, режим Discovery).
Передай: путь к проекту + `.i2c/config.md`. Пишет: `.i2c/scratch/memory-draft.md`.

**Шаг 3 — Подтверждение:** покажи черновик пользователю. Пункты `[ВЫВЕДЕНО]` требуют подтверждения. После правок скопируй в `.i2c/MEMORY.md`.

**Шаг 4 — Заполни файлы памяти:**
...
Сообщи пользователю: конфигурация завершена, MEMORY.md заполнен из аудита. Следующий шаг: `/i2c-status`.
